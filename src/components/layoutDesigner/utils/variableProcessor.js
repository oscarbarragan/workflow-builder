// src/components/layoutDesigner/utils/variableProcessor.js
...
export const variableProcessor = {
    ...
    convertUnderscoresToDots: (variables) => {
      const converted = {};
      
      Object.entries(variables).forEach(([key, value]) => {
        const dotNotationKey = key.replace(/_/g, '.');
        converted[dotNotationKey] = value;
      });
      
      return converted;
    },
  
    // Filtrar solo valores primitivos
    filterPrimitiveValues: (variables) => {
      const primitives = {};
      ...
      const processPrimitives = (obj, prefix = '') => {
        Object.entries(obj).forEach(([key, value]) => {
          ...
          if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            ...
            if (hasMetadataProps) {
              if (value.hasOwnProperty('value') && isPrimitive(value.value)) {
                primitives[cleanKey] = value.value;
              }
              return;
            } else {
              processPrimitives(value, cleanKey);
            }
          } else if (Array.isArray(value)) {
            value.forEach((item, index) => {
              const arrayKey = `${cleanKey}[${index}]`;
              if (isPrimitive(item)) {
                primitives[arrayKey] = item;
              } else if (typeof item === 'object' && item !== null) {
                processPrimitives(item, arrayKey);
              }
            });
          } else if (isPrimitive(value)) {
            primitives[cleanKey] = value;
          }
        });
      };
      
      processPrimitives(variables);
      return primitives;
    },
    ...
    processAvailableVariables: (availableVariables) => {
      ...
      // Filtrar solo primitivos
      const primitives = variableProcessor.filterPrimitiveValues(dataToProcess);
      
      // Formatear para uso en componentes
      const formatted = {};
      ...
      console.log('✅ Processed variables:', formatted);
      return formatted;
    },
