...
// Motor de evaluación para el flujo avanzado de páginas - SIN NEXT PAGE
...
/**
 * Clase principal para evaluar el flujo de páginas (solo página de inicio)
 */
export class PageFlowEngine {
  ...
  /**
   * ✅ ACTUALIZADO: Evaluar flujo de una página específica (solo página de inicio)
   */
  ...
  /**
   * ✅ ACTUALIZADO: Evaluar página simple (retorna página de inicio)
   */
  evaluateSimplePage(flowConfig, context) {
    const result = {
      type: PAGE_FLOW_TYPES.SIMPLE,
      shouldRender: true,
      startPageIndex: flowConfig.simple?.startPageIndex ?? 0, // ✅ CAMBIADO: targetPageIndex → startPageIndex
      startPageId: flowConfig.simple?.startPageId ?? null,     // ✅ CAMBIADO: targetPageId → startPageId
      context,
      iterations: 1
    };
  ...
  /**
   * ✅ ACTUALIZADO: Evaluar página condicional (retorna página de inicio según condiciones)
   */
  evaluateConditionalPage(flowConfig, context) {
    ...
    for (const condition of conditions) {
      if (this.evaluateCondition(condition, context)) {
        const result = {
          ...
          shouldRender: true,
          startPageIndex: condition.startPageIndex, // ✅ CAMBIADO: targetPageIndex → startPageIndex
          startPageId: condition.startPageId,       // ✅ CAMBIADO: targetPageId → startPageId
          context,
    ...
    const defaultStartIndex = flowConfig.conditional?.defaultStartPageIndex ?? 0; // ✅ CAMBIADO: defaultTargetPageIndex → defaultStartPageIndex
    const defaultStartId = flowConfig.conditional?.defaultStartPageId ?? null;     // ✅ CAMBIADO: defaultTargetPageId → defaultStartPageId
    
    const result = {
      type: PAGE_FLOW_TYPES.CONDITIONAL,
      shouldRender: true, // ✅ CAMBIADO: Siempre renderizar, usar página por defecto
      startPageIndex: defaultStartIndex,
      startPageId: defaultStartId,
      context,
  ...
  /**
   * ✅ ACTUALIZADO: Evaluar página repetida (retorna página de inicio para cada iteración)
   */
  evaluateRepeatedPage(flowConfig, context) {
    ...
    const result = {
      ...
      startPageIndex: repeatedConfig.startPageIndex ?? 0, // ✅ CAMBIADO: templatePageIndex → startPageIndex
      startPageId: repeatedConfig.startPageId ?? null,    // ✅ CAMBIADO: templatePageId → startPageId
      context,
  ...
  /**
   * Evaluar una condición específica (sin cambios)
   */
  ...
  /**
   * Evaluar condición basada en variable (sin cambios)
   */
  ...
  /**
   * Evaluar condición basada en script/expresión (sin cambios)
   */
  ...
  /**
   * Evaluar condición de existencia (sin cambios)
   */
  ...
  /**
   * Comparar valores usando operador específico (sin cambios)
   */
  ...
  /**
   * Obtener valor de variable usando path con notación de punto (sin cambios)
   */
  ...
  /**
   * Obtener datos para iteración en páginas repetidas (sin cambios)
   */
  ...
  // ✅ ELIMINADO: evaluateNextPage - Ya no se necesita
  
  /**
   * ✅ ACTUALIZADO: Obtener página de inicio basada en configuración
   */
  getStartPage(pageIndex, contextData = {}) {
    const pageFlow = this.evaluatePageFlow(pageIndex, contextData);
    
    if (!pageFlow) {
      return { startPageIndex: 0, startPageId: null }; // Default a primera página
    }
    
    return {
      startPageIndex: pageFlow.startPageIndex ?? 0,
      startPageId: pageFlow.startPageId ?? null,
      shouldRender: pageFlow.shouldRender,
      matchedCondition: pageFlow.matchedCondition || null,
      iterations: pageFlow.iterations || 1,
      iterationData: pageFlow.iterationData || null
    };
  }

  /**
   * Validar script por seguridad (sin cambios)
   */
  ...
  /**
   * Crear contexto seguro para evaluación (sin cambios)
   */
  ...
  /**
   * Evaluar expresión de forma segura (sin cambios)
   */
  ...
  /**
   * ✅ ACTUALIZADO: Generar secuencia de renderizado basada en página de inicio
   */
  generateRenderSequence(pageIndex = 0, contextData = {}) {
    const sequence = [];
    const pageFlow = this.evaluatePageFlow(pageIndex, contextData);
    
    if (!pageFlow || !pageFlow.shouldRender) {
      this.log('Page flow evaluation failed or page should not render', { pageIndex, pageFlow });
      return sequence;
    }
    ...
    switch (pageFlow.type) {
      case PAGE_FLOW_TYPES.SIMPLE:
        sequence.push({
          ...
          startPageIndex: pageFlow.startPageIndex,
          startPageId: pageFlow.startPageId,
          context: contextData,
          flowResult: pageFlow,
          iterationIndex: 0
        });
      ...
      case PAGE_FLOW_TYPES.CONDITIONAL:
        sequence.push({
          ...
          context: contextData,
          flowResult: pageFlow,
          matchedCondition: pageFlow.matchedCondition,
          iterationIndex: 0
        });
        break;
        
      case PAGE_FLOW_TYPES.REPEATED:
        // Para páginas repetidas, crear una entrada por cada iteración
        for (let i = 0; i < pageFlow.iterations; i++) {
          ...
          sequence.push({
            ...
            flowResult: pageFlow,
            iterationIndex: i,
            iterationData: pageFlow.iterationData[i]
          });
        }
        break;
    }
    
    this.log('Render sequence generated', {
      totalRenders: sequence.length,
      pageIndex,
      flowType: pageFlow.type
    });
  ...
  /**
   * Obtener logs de ejecución (sin cambios)
   */
  getExecutionLog() {
